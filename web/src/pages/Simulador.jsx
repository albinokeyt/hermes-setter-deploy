...
function textoEvento(e) {
  ...
  const m = {
    ...
    activacion_descartada: () => `✖ Activación descartada: ${({ conversacion_activa: 'el setter habló hace menos de 3 h (la conversación sigue viva, no se manda otro mensaje)', humano_reciente: 'una persona del equipo escribió hace poco' })[p.motivo] || p.motivo}`,
    activacion_absorbida: () => '🔕 Activación absorbida: el lead tenía un mensaje sin contestar y se le responde normal',
    activaciones_juntadas: () => `🧩 Dos peticiones casi a la vez: se juntan en un solo mensaje (etiqueta «${p.tag}»)`,
    pausa_por_humano_reciente: () => `🙋 Una persona escribió hace ${p.humano_hace_min} min: el setter se pausa y no entra`,
    activador_respeta_pausa_humana: () => `🙋 La etiqueta no despierta al setter: una persona escribió hace ${p.humano_hace_h} h`,
    seguimiento_omitido_humano_reciente: () => '🙋 Seguimiento omitido: una persona del equipo escribió hace poco',
    envio_duplicado_evitado: () => '✖ Burbuja idéntica a una enviada hace menos de 10 min: no se repite',
    externo_automatizacion_sin_pausa: () => `Mensaje de una automatización (${p.origen}): el setter no se pausa`,
    handoff_humano: () => `🙋 Una persona escribió (${p.origen}): el setter se pausa`,
    saliente_origen_desconocido: () => 'GHL no dijo quién mandó el mensaje: se trata como persona',
    ...
    seguimiento_acuerdo_fuera_ventana: () => `⏳ Prometió escribir en ${p.horas_acordadas} h y eso no cabe en la ventana de Meta (quedan ${p.ventana_restante_h} h): no se escribe antes de lo prometido`,
