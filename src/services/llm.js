...
export async function chatCompletion({ provider, model, temperature = 0.8, messages, maxTokens = 900, json = true }) {
  ...
  if (!res.ok) throw new LlmError(`LLM ${provider.name} → ${res.status}: ${text.slice(0, 400)}`, res.status, data);

  const content = data?.choices?.[0]?.message?.content;
  if (typeof content !== 'string' || !content.trim()) {
    throw new LlmError(`LLM ${provider.name} devolvió una respuesta vacía`, 502, data);
  }
  return content;
}
