...
import { processDebounce, processSend, processFollowup, logEvent } from './services/pipeline.js';
...
export function startWorkers() {
  ...
  for (const w of [debounceWorker, sendWorker, followupWorker]) {
    w.on('failed', (job, err) => {
  ...
  console.log('[workers] debounce, send y followup en marcha');
  return { debounceWorker, sendWorker, followupWorker };
}
